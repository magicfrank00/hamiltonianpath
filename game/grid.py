...
def generate_map(grid_size):
    ...
    start_x, start_y = random.choice([(0, random.randint(0, grid_size - 1)),
    ...
    grid[start_y][start_x] = (random.randint(200, 255), random.randint(100, 150), random.randint(100, 150))
    ...
    for i in range(len(path)):
        x, y = path[i]
        if not has_unvisited_neighbors(x, y) and len(path) < grid_size * grid_size * 0.6:
            if i != 0:
                path = path[:i]  # Cut off the invalid portion of the path
                break
    # print the path as directions to take
...
def find_solution(grid, start_position, start_time, time_limit=1):
    ...
    def is_valid_move(x, y):
        return 0 <= x < grid_size and 0 <= y < grid_size and grid[y][x] is not None and (x, y) not in visited
    ...
    def backtrack(x, y):
        ...
        if len(visited) == sum(sum(1 for cell in row if cell is not None) for row in grid):
            return True
